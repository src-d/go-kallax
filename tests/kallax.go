...
func NewFoo() (record *Foo) {
	return new(Foo)
}

// GetID returns the primary key of the model.
func (r *Foo) GetID() kallax.Identifier {
	return (*kallax.NumericID)(&r.ID)
}

// ColumnAddress returns the pointer to the value of the given column.
func (r *Foo) ColumnAddress(col string) (interface{}, error) {
	switch col {
	case "id":
		return (*kallax.NumericID)(&r.ID), nil

	default:
		return nil, fmt.Errorf("kallax: invalid column in Foo: %s", col)
	}
}

// Value returns the value of the given column.
func (r *Foo) Value(col string) (interface{}, error) {
	switch col {
	case "id":
		return r.ID, nil
...
// NewRelationshipRecord returns a new record for the relatiobship in the given
// field.
func (r *Foo) NewRelationshipRecord(field string) (kallax.Record, error) {
	return nil, fmt.Errorf("kallax: model Foo has no relationships")
}

// SetRelationship sets the given relationship in the given field.
func (r *Foo) SetRelationship(field string, rel interface{}) error {
	return fmt.Errorf("kallax: model Foo has no relationships")
}

// FooStore is the entity to access the records of the type Foo
// in the database.
type FooStore struct {
	*kallax.Store
}

// NewFooStore creates a new instance of FooStore
// using a SQL database.
func NewFooStore(db *sql.DB) *FooStore {
	return &FooStore{kallax.NewStore(db)}
}

// GenericStore returns the generic store of this store.
func (s *FooStore) GenericStore() *kallax.Store {
	return s.Store
}

// SetGenericStore changes the generic store of this store.
func (s *FooStore) SetGenericStore(store *kallax.Store) {
	s.Store = store
}

// Debug returns a new store that will print all SQL statements to stdout using
// the log.Printf function.
func (s *FooStore) Debug() *FooStore {
	return &FooStore{s.Store.Debug()}
}

// DebugWith returns a new store that will print all SQL statements using the
// given logger function.
func (s *FooStore) DebugWith(logger kallax.LoggerFunc) *FooStore {
	return &FooStore{s.Store.DebugWith(logger)}
}

// Insert inserts a Foo in the database. A non-persisted object is
// required for this operation.
func (s *FooStore) Insert(record *Foo) error {
	return s.Store.Insert(Schema.Foo.BaseSchema, record)
}
...
// Only writable records can be updated. Writable objects are those that have
// been just inserted or retrieved using a query with no custom select fields.
func (s *FooStore) Update(record *Foo, cols ...kallax.SchemaField) (updated int64, err error) {
	return s.Store.Update(Schema.Foo.BaseSchema, record, cols...)
}

// Save inserts the object if the record is not persisted, otherwise it updates
// it. Same rules of Update and Insert apply depending on the case.
func (s *FooStore) Save(record *Foo) (updated bool, err error) {
	if !record.IsPersisted() {
		return false, s.Insert(record)
	}
...
// Delete removes the given record from the database.
func (s *FooStore) Delete(record *Foo) error {
	return s.Store.Delete(Schema.Foo.BaseSchema, record)
}

// Find returns the set of results for the given query.
func (s *FooStore) Find(q *FooQuery) (*FooResultSet, error) {
	rs, err := s.Store.Find(q)
	if err != nil {
		return nil, err
	}

	return NewFooResultSet(rs), nil
}

// MustFind returns the set of results for the given query, but panics if there
// is any error.
func (s *FooStore) MustFind(q *FooQuery) *FooResultSet {
	return NewFooResultSet(s.Store.MustFind(q))
}

// Count returns the number of rows that would be retrieved with the given
// query.
func (s *FooStore) Count(q *FooQuery) (int64, error) {
	return s.Store.Count(q)
}

// MustCount returns the number of rows that would be retrieved with the given
// query, but panics if there is an error.
func (s *FooStore) MustCount(q *FooQuery) int64 {
	return s.Store.MustCount(q)
}

// FindOne returns the first row returned by the given query.
// `ErrNotFound` is returned if there are no results.
func (s *FooStore) FindOne(q *FooQuery) (*Foo, error) {
	q.Limit(1)
	q.Offset(0)
	rs, err := s.Find(q)
	if err != nil {
...
// FindAll returns a list of all the rows returned by the given query.
func (s *FooStore) FindAll(q *FooQuery) ([]*Foo, error) {
	rs, err := s.Find(q)
	if err != nil {
		return nil, err
	}
...
// MustFindOne returns the first row retrieved by the given query. It panics
// if there is an error or if there are no rows.
func (s *FooStore) MustFindOne(q *FooQuery) *Foo {
	record, err := s.FindOne(q)
	if err != nil {
		panic(err)
	}
...
// Reload refreshes the Foo with the data in the database and
// makes it writable.
func (s *FooStore) Reload(record *Foo) error {
	return s.Store.Reload(Schema.Foo.BaseSchema, record)
}
...
// The transaction is only open in the store passed as a parameter to the
// callback.
func (s *FooStore) Transaction(callback func(*FooStore) error) error {
	if callback == nil {
		return kallax.ErrInvalidTxCallback
	}

	return s.Store.Transaction(func(store *kallax.Store) error {
		return callback(&FooStore{store})
	})
}

// FooQuery is the object used to create queries for the Foo
// entity.
type FooQuery struct {
	*kallax.BaseQuery
}

// NewFooQuery returns a new instance of FooQuery.
func NewFooQuery() *FooQuery {
	return &FooQuery{
		BaseQuery: kallax.NewBaseQuery(Schema.Foo.BaseSchema),
	}
}

// Select adds columns to select in the query.
func (q *FooQuery) Select(columns ...kallax.SchemaField) *FooQuery {
	if len(columns) == 0 {
		return q
	}
	q.BaseQuery.Select(columns...)
...
// SelectNot excludes columns from being selected in the query.
func (q *FooQuery) SelectNot(columns ...kallax.SchemaField) *FooQuery {
	q.BaseQuery.SelectNot(columns...)
	return q
}
...
// Order adds order clauses to the query for the given columns.
func (q *FooQuery) Order(cols ...kallax.ColumnOrder) *FooQuery {
	q.BaseQuery.Order(cols...)
	return q
}
...
// BatchSize sets the number of items to fetch per batch when there are 1:N
// relationships selected in the query.
func (q *FooQuery) BatchSize(size uint64) *FooQuery {
	q.BaseQuery.BatchSize(size)
	return q
}

// Limit sets the max number of items to retrieve.
func (q *FooQuery) Limit(n uint64) *FooQuery {
	q.BaseQuery.Limit(n)
	return q
}

// Offset sets the number of items to skip from the result set of items.
func (q *FooQuery) Offset(n uint64) *FooQuery {
	q.BaseQuery.Offset(n)
	return q
}
...
// Where adds a condition to the query. All conditions added are concatenated
// using a logical AND.
func (q *FooQuery) Where(cond kallax.Condition) *FooQuery {
	q.BaseQuery.Where(cond)
	return q
}
...
// the ID property is equal to one of the passed values; if no passed values,
// it will do nothing.
func (q *FooQuery) FindByID(v ...int64) *FooQuery {
	if len(v) == 0 {
		return q
	}
	values := make([]interface{}, len(v))
	...
	return q.Where(kallax.In(Schema.Foo.ID, values...))
}

// FooResultSet is the set of results returned by a query to the
// database.
type FooResultSet struct {
	ResultSet kallax.ResultSet
	last      *Foo
	lastErr   error
}

// NewFooResultSet creates a new result set for rows of the type
// Foo.
func NewFooResultSet(rs kallax.ResultSet) *FooResultSet {
	return &FooResultSet{ResultSet: rs}
}

// Next fetches the next item in the result set and returns true if there is
// a next item.
// The result set is closed automatically when there are no more items.
func (rs *FooResultSet) Next() bool {
	if !rs.ResultSet.Next() {
		rs.lastErr = rs.ResultSet.Close()
		rs.last = nil
		return false
	...
	var record kallax.Record
	record, rs.lastErr = rs.ResultSet.Get(Schema.Foo.BaseSchema)
	if rs.lastErr != nil {
		rs.last = nil
	} else {
		var ok bool
		rs.last, ok = record.(*Foo)
		if !ok {
			rs.lastErr = fmt.Errorf("kallax: unable to convert record to *Foo")
			rs.last = nil
		}
	}
...
// Get retrieves the last fetched item from the result set and the last error.
func (rs *FooResultSet) Get() (*Foo, error) {
	return rs.last, rs.lastErr
}
...
// `kallax.ErrStop` in the callback.
// Result set is always closed at the end.
func (rs *FooResultSet) ForEach(fn func(*Foo) error) error {
	for rs.Next() {
		record, err := rs.Get()
		if err != nil {
			return err
...
// All returns all records on the result set and closes the result set.
func (rs *FooResultSet) All() ([]*Foo, error) {
	var result []*Foo
	for rs.Next() {
		record, err := rs.Get()
		if err != nil {
...
// One returns the first record on the result set and closes the result set.
func (rs *FooResultSet) One() (*Foo, error) {
	if !rs.Next() {
		return nil, kallax.ErrNotFound
	}
...
// Err returns the last error occurred.
func (rs *FooResultSet) Err() error {
	return rs.lastErr
}

// Close closes the result set.
func (rs *FooResultSet) Close() error {
	return rs.ResultSet.Close()
}
